...
const config = {
  ...
  // Absolute cap on a session's lifetime regardless of activity - the
  // frontend's cookies slide the actual 1h *idle* timeout forward on every
  // authenticated request (see frontend/lib/auth/cookies.ts's
  // touchSessionCookies), so this only kicks in for a session active
